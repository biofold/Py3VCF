    ...
    def _parse_info(self, info_str):
        ...
        for entry in entries:
            ...
            try:
                ...
                ## Modified by Zauli: if type is float, but we have a list with string ['.,0.852', '0.91'] 
                ## o list with mixed "|" and "," that are difficult to manage, change the type in String
                if entry[1:] and entry_type == 'Float' and \
                    (  ",." in str(entry[1:]) or ".," in str(entry[1:]) or \
                       ".|" in str(entry[1:])  or ".|" in str(entry[1:]) or \
                       ( "|" in str(entry[1:]) and "." in str(entry[1:]) ) \
                    ) :
                        entry_type = 'String'
            except KeyError:
                try:
            ...
            if entry_type == 'Integer':
                vals = entry[1].split(',')
                try:
                    ## Modified by Zauli to manage list of Values with a pipe separator
                    if "|" in entry[1]:
                        entry_type == 'String'
                        vals = entry[1].split('|')
                ...
                # Handles cases with incorrectly specified header types.
                    ...
                    ## switch to a String, damned dbNSFP random types
                        val = self._map(str, vals)
            elif entry_type == 'Float':
                ## Modified by Zauli to manage list of Values with a pipe separator
                if "_" in entry[1]:
                    ##problems on parsing clinvar_clnsig with values like  3_3 on single variants.
                    ##probably if multipple transcript it produce a value like the one above. But it seems to don't have any sense
